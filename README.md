...
| アプローチ | 活用場面 | 強み | 弱み, ハマりどころ | 当ライブラリとの関係 |
|---|---|---|---|---|
| **GraphicRaycaster の BlockingObjects** | UI 同士のクリック貫通防止, 背面ブロック | 標準機能で簡単、入力まわり統制に強い | 視覚的重なりは解決しない(見た目そのまま)。非UIとの見た目の干渉は直接扱えない | **補完関係**。入力制御は Raycaster、視認性(見た目の重なり検出)は本ライブラリ |
| **CanvasGroup(alpha, blocksRaycasts)** | UI の**透明度, 入力**を一括制御 | 実装が軽い。ポリシーを１箇所に集約 | 切り替えの**トリガー**が必要 | **相性が良い**。本ライブラリのイベント をトリガーに CanvasGroup を操作 |
...
| **SpriteMask, RectMask2D, Mask** | 形状に基づく**切り抜き** | 表現力が高い | セットアップ, デザイン調整のコストが高い。UI と SpriteMask はレンダリング経路が別 | **目的が異なる**(切り抜き表現)。視認性確保だけならフェードの方が副作用が小さい |
